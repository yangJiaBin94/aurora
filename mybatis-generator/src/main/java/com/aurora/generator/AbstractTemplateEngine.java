...
public abstract class AbstractTemplateEngine {
     ...
     * 配置信息
     */
    private ConfigBuilderV2 configBuilder;


    /**
     * 模板引擎初始化
     */
    public AbstractTemplateEngine init(ConfigBuilderV2 configBuilder) {
     ...
     */
    public void open() {
        String daoOutDir = getConfigBuilder().getGlobalConfig().getDaoOutputDir();
        String serviceOutDir = getConfigBuilder().getGlobalConfig().getServiceOutputDir();
        if (getConfigBuilder().getGlobalConfig().isOpen()) {
            if (StringUtils.isNotEmpty(daoOutDir)) {
                openDir(daoOutDir);
            }
            if (StringUtils.isNotEmpty(serviceOutDir) && !serviceOutDir.equals(daoOutDir)) {
                openDir(serviceOutDir);
            }

        }
    }

    private void openDir(String outDir) {
        try {
    ...
    public Map<String, Object> getObjectMap(TableInfo tableInfo) {
        ...
        ConfigBuilderV2 config = getConfigBuilder();
    ...
    protected boolean isCreate(FileType fileType, String filePath) {
        ConfigBuilderV2 cb = getConfigBuilder();
        // 自定义判断
    ...
    public ConfigBuilderV2 getConfigBuilder() {
        return configBuilder;
    }

    public AbstractTemplateEngine setConfigBuilder(ConfigBuilderV2 configBuilder) {
        this.configBuilder = configBuilder;
        return this;
    }
}
